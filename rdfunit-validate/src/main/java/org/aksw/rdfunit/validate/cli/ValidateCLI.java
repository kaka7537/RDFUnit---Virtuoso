...
import org.aksw.rdfunit.virtuoso.VirtuosoExecutor;
...
public class ValidateCLI {
     ...
     */
    public static void doMain(String[] args, String filename_parameter) throws Exception {

        CommandLine commandLine = ValidateUtils.parseArguments(args);
//System.out.println("Command LINE = " + commandLine.toString() + "ARGS = " + args[0]);
	...
	System.out.println("Execution Type = " + configuration.getTestCaseExecutionType());
	...
	testModel.read(filename_parameter, "TTL");
	System.out.println(filename_parameter + " TTL FILE OPEN SUCCESS!");
        // warning, caches intermediate results
        testExecutor.execute(dataset, testSuite, testModel);
        TestExecution testExecution = testExecutorMonitor.getTestExecution();

	FileOutputStream fos;
	try {
		fos = new FileOutputStream(filename_parameter);
		testModel.write(new OutputStreamWriter(fos, "UTF8"), "TURTLE");
